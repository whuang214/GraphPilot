...
> **Status:** Historical readiness architecture snapshot. Current intended behavior is owned by
> [`01-readiness-reviewer.md`](../../../03-design/01-context-generation/01-readiness/02-reviewer.md);
...
> policy, recommendations, bounded improve-before-ask loop, and user override boundary. Exact facets,
> applicability, anchors, weights, assumption cap, finding registry, structured reviewer output validation,
> score/rounding/status algorithm, fixtures, calibration, and repeated-run stability belong only to 05. The
> overall flow is [`01-workflows-and-prompts.md`](../01-workflows-and-prompts.md); JSON 1 and JSON 2 structures
> are [`02-evidence-manifest.md`](../02-evidence-manifest.md) and
...
adequate. Under the v1 **Option A** boundary, the separate backend generator—not the host or reviewer—later
...
The private review JSON is documented in [`03-reviewer-json.md`](03-reviewer-json.md). The one final result
JSON is documented in [`04-results.md`](04-results.md). Neither is persisted inside JSON 1 or JSON 2.
...
2. required and conditionally applicable per-type facets satisfy the exact 05 rubric;
...
No historical versions, raw evidence, discovery graph, or graph-native scores are included. If the index
exceeds the future reviewer budget, deterministic filtering/retrieval/chunking must preserve omission
checking; the exact large-manifest policy remains an implementation prerequisite.

### Other reviewer inputs

- JSON 2 request framing, scope, assumptions, decisions, and current uncertainty dispositions;
- the selected diagram type and exact versioned rubric from 05;
- an allowlist of selected and indexed claim IDs/versions, uncertainty IDs, and assumption IDs;
...
- every required unselected payload claim reference is returned as a deterministic `context` selection
  recommendation; the backend never silently adds it to JSON 2.
...
- every assumption is referenced by exactly one `assume` disposition and has a valid `origin`, `acceptedBy`,
  and `acceptedAt`;
- every `exclude` disposition has a corresponding request-scope exclusion; one exclusion may justify
  multiple dispositions, while exclusions unrelated to an open uncertainty need no disposition;
- every `defer` disposition remains advisory unless Layers 2/3 determine it affects required in-scope
  meaning;
- `block` dispositions are hard readiness blockers;
- assumptions never masquerade as JSON 1 claims.

#### Safety and bounds

- JSON 2 contains no raw source, discovery output, or copied manifest history;
- input sizes stay within configured limits;
- no secrets or forbidden content are forwarded;
- reviewer allowlists are complete and internally consistent.

A Layer 1 failure produces `invalid` when a safe readiness result can be formed; it is never sent to the LLM
and cannot be overridden. Transport/load/path failures that prevent a safe result remain operation errors.

### Layer 2 — LLM per-type rubric evaluation

The backend selects the immutable 05 rubric for `request.diagramType`. For each facet, the reviewer returns
only semantic judgments: applicability, rating when applicable, exact supporting claim/assumption refs, and
a bounded rationale. It does not return weights, derived facet status, severity, overrideability, score, or
overall status.

Required versus conditional behavior, all facet IDs/criteria, `0..4` anchors, assumption handling, and valid
field combinations are defined only in
[`05-readiness/`](README.md).
...
Exact codes, severity, overrideability, consequence/action semantics, and raw item shapes belong only to 05.

### Layer 4 — deterministic validation and aggregation

The LLM is advisory. Layer 4 validates the strict raw structured result against the exact 05 rules:
...
- recommendations use active/current claims and valid roles;
...
If reviewer output is invalid, one repair call receives validation diagnostics. A second invalid response
returns an operation error; a valid but unfavorable judgment is never retried.
...
3. calculates the secondary score using exact 05 rounding;
...
05; this section owns how their result gates the workflow.
...
coverage exists: `readinessScore` is `null` and `coverage` is empty.

### Score role

For a successful Layer 2/3 review, Layer 4 returns the exact 05 weighted integer score over applicable and
...
- non-overrideable coverage or holistic findings from 05;
...
Only blockers whose exact 05 registry policy has `overrideable: true`, such as a secondary conditional facet
...
2 assumption under the host/user consultation policy owned by
[`../07-mcp-prompts-and-tool-contracts.md`](../07-mcp-prompts-and-tool-contracts.md); readiness never creates
or re-approves that assumption.

### Working override information

Exact per-attempt transport lives in
[`../07-mcp-prompts-and-tool-contracts.md`](../07-mcp-prompts-and-tool-contracts.md). The readiness boundary
...
It is consumed by final generation readiness and summarized in canonical trace metadata.
...
This policy is attempt-specific and deliberately not a JSON 2 field. Its digest-bound call shape and canonical
trace behavior are defined in `07` without changing the override boundary above.
...
[`04-results.md`](04-results.md). This reviewer document owns when that result is produced and how the host
...
3. **Ask only when needed.** Escalate when repository context cannot settle a factual ambiguity, alternatives
   ...
   consultation happens before the accepted assumption is persisted, under the `07` host prompt.
...
consult under the adaptive 07 policy and submit a separate `allow_warnings` attempt. An already explicit
truthful-best-effort preference may permit the host to choose that mode without another omission question.
Persisted assumptions are already accepted; readiness keeps their warnings visible but does not re-evaluate
who accepted them.
...
- choose policy fields owned by 05/Layer 4;
- bypass deterministic safety, provenance, validation, or structural rules.

## Errors (not statuses)

Operational failures return tool errors rather than readiness statuses:
...
- reviewer projection/token budget exceeded without a safe bounded strategy;
- unexpected internal failure.

When Azure semantic review is unavailable, readiness returns a retryable operation error and context-backed
generation does not proceed. Deterministic-only and force-skip fallbacks are forbidden.

## Historical implementation follow-through

The active readiness package owns the promoted contract. Exact reviewer deployment/timeouts/budgets,
deployment-change recertification, reviewer/generator deployment selection, runtime services, and machine
schemas are assigned to Epic 3's group-local readiness and release slices rather than this snapshot.

## Active dependencies

1. [`05-generation-and-provenance.md`](../../../03-design/01-context-generation/05-generation-and-provenance.md)
   consumes the final readiness interface and defines compact trace metadata;
2. [`03-mcp-tools/`](../../../02-architecture/01-mcp-tools/README.md) owns public transport and host orchestration;
3. [`04-context-backed-generation/`](../../../07-history/epics/03-mcp-generation-and-wrappers/04-context-backed-generation/00-group.md)
...
> assesses JSON 2 against the exact 05 per-type rubric and supported JSON 1 knowledge; it returns one final
> result with authoritative status, secondary score, grounded grades, structured findings/consequences/actions,
> and bounded override information. The host improves JSON 1/JSON 2 automatically before asking the user,
> without source fact-checking by the reviewer, invented facts, silent context mutation, unchanged-input
> rerolls, or diagram generation.
