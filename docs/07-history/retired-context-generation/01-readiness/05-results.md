...
> [`03-reviewer-json.md`](04-reviewer-json.md); deterministic calculation lives in
> [`02-policy-and-calculation.md`](03-policy-and-calculation.md); the overall flow and host loop live in
> [`01-readiness-reviewer.md`](02-reviewer.md). Exact public tool transport belongs to the
...
```json
{
  "schemaVersion": "graphpilot.context.readiness-result.v1",
...
| --- | --- | --- |
| `schemaVersion` | Yes | Constant `graphpilot.context.readiness-result.v1`. |
...
| `summary` | Yes | Concise deterministic human/host-readable result assembled by the backend. |
| `invalidIssues` | Yes | Sorted Layer 1 `{code,message,path}` issues; nonempty only for `invalid`, otherwise empty. |
...
| Field | Meaning |
| --- | --- |
| `id` | Deterministic `finding-<three-digit final-order ordinal>-<code>` identity; canonical finding order fixes the ordinal. |
...
| `recommendedAction` | Exact discriminated action payload from [`03-reviewer-json.md`](04-reviewer-json.md); the host decides and performs it. |

## Status behavior

| Status | Final-result behavior |
| --- | --- |
| `invalid` | Null score, nonempty `invalidIssues`, empty coverage, no generation or override. |
| `needs_context` | One or more blockers; host follows actions and reassesses after actual context change. |
| `ready_with_warnings` | No blockers; `canGenerate` remains false until a separate generation attempt explicitly accepts warnings under the [`01-readiness-reviewer.md`](02-reviewer.md) policy. |
| `ready` | No blocker or warning; `canGenerate` is true. |

A reviewer/tool operation failure is not serialized as one of these statuses; it uses `error: OperationProblem`
and MCP `isError: true` at the tool boundary.

## V1 bounds and ordering

The decoded final result is at most 2 MiB and inherits the private review's text/item bounds. `invalidIssues` has
at most 256 entries: after sorting by path/code/message, more than 256 becomes the first 255 plus one
`additional_issues_omitted` entry whose message reports only the omitted count. This diagnostic reduction is
explicit, occurs only after readiness is already `invalid`, and is never reviewer input. Coverage follows rubric
order; final findings follow deterministic policy order; claim-bearing arrays sort by `(id, version)`; and
uncertainty/assumption/question arrays sort by ID. Bounds may increase but not decrease within v1.

## Validation invariants

1. Envelope constants and request ID/digest, manifest digest, type, and rubric identities are exact.
2. `invalid` has nonempty sorted `invalidIssues`, null score, empty coverage/findings/recommendations, and false
   generation/override flags; every other status has empty `invalidIssues`.
3. Status/score/coverage combinations obey deterministic policy.
4. Coverage exactly matches the selected rubric in canonical order.
5. Final findings contain only registered policy fields and allowlisted refs.
6. Every final action preserves its validated discriminated shape and exact allowlisted references.
7. Recommendations/actions cross-reference the same exact claims, uncertainties, assumptions, and question IDs.
8. `canGenerate` and `canOverride` follow the 01 reviewer policy and cannot be chosen by the LLM.
9. The final result is returned to the host but not embedded into JSON 1 or JSON 2.
