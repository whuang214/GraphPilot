...
> [`02-policy-and-calculation.md`](03-policy-and-calculation.md); the final host-facing readiness-result
> envelope remains owned by [`04-results.md`](05-results.md).
...
```json
{
  ...
  "rubricVersion": "graphpilot.context.readiness-rubric.use-case.v1",
  "coverage": [
    {
...
| `rating` | Yes | Integer `0..4` when applicable; explicit `null` for `not_applicable` or `uncertain` so Azure strict output can require every property. |
| `supportingClaimRefs` | Yes | Unique exact selected claim refs; unselected index claims cannot support a rating; must be empty when applicability is `not_applicable` or `uncertain`. |
| `assumptionRefs` | Yes | Unique exact accepted JSON 2 assumption IDs; must be empty when applicability is `not_applicable` or `uncertain`. |
...
[`02-policy-and-calculation.md`](03-policy-and-calculation.md). `coverage` contains every rubric facet exactly
...
| `uncertainty_disposition_blocks` | `uncertainty` | `blocking` | No | JSON 2 explicitly assigns `block` to an open uncertainty. | `search_source` or `ask_user` using the exact uncertainty. |
| `assumption_backed_coverage` | `assumption` | `warning` | No | A facet rating `1..3` cites an accepted assumption. | `search_source` when support may replace it; warning acceptance otherwise follows [`01-readiness-reviewer.md`](02-reviewer.md). |
...
non-modelable. `selection_closure_missing` is synthesized from Layer 1's exact direct-reference requirement rather
than selected by the reviewer; its exact `select_existing_claim` action is sufficient and does not require a
reviewer-authored top-level `recommendedSelections` item with guessed facet coverage.
...
and how to execute them. Exact execution and presentation belong to the
[unified MCP generation workflow](../../../02-architecture/01-mcp-tools/README.md); tool transport belongs to
the [MCP context-tool contract](../../../02-architecture/01-mcp-tools/02-context-tools.md).
...
| --- | --- | --- |
| `select_existing_claim` | `claimRefs` | Non-empty refs from the explicit active/current `unselectedIndexedClaimRefs` allowlist; no other payload fields. |
...
| --- | --- |
| `recommendedSelections` | `claimRef` from the explicit `unselectedIndexedClaimRefs` projection allowlist, JSON 2 `role`, non-empty `reason`, non-empty valid `supportsFacets`; empty when that allowlist is empty. |
...
[`04-results.md`](05-results.md).
The host therefore consumes one result instead of separate grade, warning, and action calls. Exact host
wording belongs to the
[unified MCP generation workflow](../../../02-architecture/01-mcp-tools/README.md).

## 7. V1 bounds and canonical order

The decoded private review object is at most 1 MiB. Semantic message, rationale, consequence, reason,
description, action-description, and question strings are nonblank and at most 2,000 characters; suggested-search
strings are at most 512 characters. `coverage` contains at most 9 items; `holisticFindings`,
`recommendedSelections`, `removeSelections`, `missingContext`, and `questions` contain at most 64 each;
`relevantUncertainties` contains at most 256; and any containing claim/uncertainty/assumption/search/ref array
contains at most 256 except suggested searches, which contain at most 32. Question IDs use
`^question-[a-z0-9]+(?:-[a-z0-9]+)*$` and at most 128 characters. Bounds may increase but not decrease within v1.

After validation the backend orders coverage by rubric; claim-bearing items by `(id, version)`; uncertainty,
assumption, and question-bearing items by ID; missing context by affected-facet rubric order then description; and
holistic findings by rubric facet then code and sorted refs. Array order from the model never controls policy or
final-result determinism.

## 8. Deterministic structured-output validation
...
19. reviewer-authored omission findings have matching selection recommendations; deterministic
    `selection_closure_missing` instead has its exact `select_existing_claim` action;
...
23. every applicable facet below its minimum has a matching recommendation, missing-context item, relevant
    uncertainty, or question from which Layer 4 can derive one typed action;
24. uncertain applicability has matching gap/uncertainty/question detail;
25. no Layer 4 policy field appears in raw output;
26. configured item/text limits are respected.

The repair prompt receives only diagnostics and identical canonical review input. It uses the same explicit selected/unselected/uncertainty/assumption/question allowlists and finding-action policies as the first response; an empty unselected list requires empty recommendations. A second invalid response returns the reviewer operation error owned by `01-readiness-reviewer.md`. A valid but unfavorable judgment is never retried.
